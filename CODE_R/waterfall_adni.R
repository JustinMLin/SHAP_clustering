...
shap_dt <- read.csv("DATA/model comparison/ADNI/adni_shap_dt.csv")
shap_xgb <- read.csv("DATA/model comparison/ADNI/adni_shap_xgb.csv")
shap_nn <- read.csv("DATA/model comparison/ADNI/adni_shap_nn.csv")

cluster <- read.csv("DATA/model comparison/ADNI/adni_hdbscan.csv")[,1]
cluster_no_na <- read.csv("DATA/model comparison/ADNI/adni_hdbscan_no_na.csv")[,1]
...
waterfall_plot <- function(shap, cluster, n_feat, title, adj, vjust=1.5, hjust=1.5, labels=TRUE) {
  ...
  mean_dist <- long_data %>%
    mutate(dist=sqrt(ClassCN^2 + ClassMCI^2 + ClassAD^2)) %>%
    group_by(feature) %>%
  ...
  means_data <- long_data %>%
    ...
    dplyr::summarize(mean0 = mean(ClassCN),
                     mean1 = mean(ClassMCI),
                     mean2 = mean(ClassAD),
                     .groups="keep") %>%
    ungroup(feature) %>%
...
waterfall_plot(shap_dt, cluster, 5, "Decision Tree", 0.2, labels=T)
waterfall_plot(shap_xgb, cluster, 5, "XGBoost", 0.8, labels=T)
waterfall_plot(shap_nn, cluster_no_na, 5, "Neural Network", 0.1, labels=T)
